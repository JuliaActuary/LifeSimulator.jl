...
function read_savings_model(model = "ME_EX4"; dir = joinpath(@__DIR__, "lifelib"))
  mx = pyimport("modelx")
...
@testset "LifelibSavings model" begin
  ...
  @testset "Importing policy sets" begin
    policies = policies_from_csv("savings/model_point_table_9.csv")
    policies_py = policies_from_csv(proj)
    @test policies == policies_py
  end

  @testset "Setting policies in Python" begin
    policies_prev = policies_from_csv(proj)
    policies = rand(PolicySet, 100)
    use_policies!(proj, policies)
    retrieved = policies_from_csv(proj)
    @test retrieved == policies
    use_policies!(proj, policies_prev)
    @test policies_from_csv(proj) == policies_prev
  end
